...
ggplot(filter(screen.vision.myr1, !is.na(test)), aes(x=test, y=score, fill=test))+
  geom_violin(adjust=1.5, draw_quantiles = 0.5, size=0.5)+
  ...
  scale_y_continuous(limits=c(-0.6, 0.7), breaks=seq(-1,1,0.5))+
...
ggsave(filename = "../outputs/fig_3/FIG_3A_GRA59_MYR1_SIGNATURE_SCORE.png", width = 4.4, height = 2.9, dpi=300)
